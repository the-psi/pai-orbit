...
- [ ] `kiro-power` ships with no agent or hook fidelity (`❌`/`❌` in `plugins/pai-orbit/README.md`'s adapter fidelity table) — the gap `codex` carried until the 2026-07-30 upgrade above, and the one `copilot` narrowed to hooks alone (`✅` agents, `⚠️` hooks) in the prompt-files upgrade. Accepted at introduction per `docs/decisions/2026-08-18-add-kiro-power-adapter.md` rather than blocking the adapter's addition; revisit when Kiro gains an agent/hook-equivalent primitive. — owner: unassigned
