...
Runs in three gated phases: establish purpose (propose from epic/ux/domain or ask), confirm each scenario in scope (granularity: distinct if acceptance criteria differ), then analyze requirements. Session-close pre-flight blocks groomed status if purpose or scenarios are incomplete; new scenarios in Phase 3 return to Phase 2. Scopes to minimal deliverable. Captures open questions with owners. Does not design solutions.
