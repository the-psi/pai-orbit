...
| D13 | **PARTIALLY SUPERSEDED 2026-07-04.** Originally dropped both `/setup` and `/suggest-skills` from the Copilot output. **Superseded (D13-rev):** `/setup` is now emitted as an agent-mode prompt (`mode: agent` + `tools: [codebase, editFiles, runCommands, search]`) so Copilot Business teams can invoke `/setup` from Chat for re-configuration. Copilot Free degrades gracefully — `/setup` loads as advisory text explaining the steps and pointing users at `npx github:the-psi/pai-orbit init copilot`. **`/suggest-skills` stays dropped** — it introspects Claude Code's plugin runtime, which has no Copilot analogue. The CLI has also been extended (2026-07-04) to full parity with `/setup` Step 2 (all 11 questions, plus Step 2b live board column discovery via `glab api` / `gh project field-list` / `linear team list`) so terminal-only teams get the same interview depth. **Result:** 13 mode prompts (was 12) → total 26 in Copilot's picker. | User feedback (2026-07-04) — parity with Claude Code was the stronger design goal than the "chicken-and-egg" argument that motivated the original drop. `/setup` in Chat gives Copilot Business teams a familiar entry point; the CLI still handles first-time install (which is prerequisite anyway). |
