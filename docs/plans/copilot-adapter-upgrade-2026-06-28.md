...
| D35 | (2026-07-05) Emit named sub-agents (`core/agents/*.md`) as Copilot `mode: agent` prompts with per-agent tool sets — `/docs-writer` gets full tools (edits `docs/`); `/cross-repo-impact` gets read-only tools (`codebase` + `search`, no `editFiles` / `runCommands`) matching the source agent's read-only contract | Named sub-agents were previously missing from Copilot dist output (fidelity gap surfaced 2026-07-05 audit). Claude Code and Cursor emit them; Copilot didn't. Adding them as agent-mode prompts closes the gap: `[agent]` prompt count goes from 7 (service-builders only) to 9 (7 + 2 named). Per-agent tool differentiation preserves the read-only contract of `cross-repo-impact` — Copilot's agent runtime respects the tools list, so Business tier won't propose file edits from that prompt. |
| D36 | (2026-07-05) Emit `core/templates/rules/decisions.md` as `.github/instructions/decisions.instructions.md` with `applyTo: "**/*"` — always attached | ADR obligation was previously missing from Copilot dist output (fidelity gap surfaced 2026-07-05 audit). Cursor-plugin emits it as `rules/decisions.mdc` with `alwaysApply: true`; Claude Code emits it as `templates/rules/decisions.md` and references it from its always-on rules directory. Copilot equivalent is an instructions file with the broadest applyTo glob — same effect, same authority. Instructions file count goes 4 → 5. |
