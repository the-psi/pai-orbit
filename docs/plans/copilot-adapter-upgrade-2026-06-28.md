...
| D13 | Drop `/setup` and `/suggest-skills` modes from the Copilot output entirely (filesystem audit 2026-06-28 confirmed both live in `core/modes/`, not `core/skills/`) | `/setup`'s purpose is replaced by the npx CLI. `/suggest-skills` is a Claude-Code introspection feature with no Copilot equivalent. Better to ship 12 working mode prompts than 14 with two confusing dead ends. |
