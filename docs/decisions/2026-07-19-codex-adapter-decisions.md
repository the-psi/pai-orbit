...
- **Cosmetic: extractor uses `os.path.join`** — produces mixed slashes on Windows (e.g. `/tmp/proj\src/main.py`) in log output. Doesn't affect behavior. Switch to `posixpath.join` for cosmetic consistency.
