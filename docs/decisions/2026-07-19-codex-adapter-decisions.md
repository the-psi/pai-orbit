...
- **Security: `install.js` `copyRecursive` follows symlinks.** `fs.copyFileSync` (install.js line ~78, per DC7) follows symlinks by default. Low risk under the current trust model (user installing tooling into their own project), but if a malicious symlink existed at a target path, the copy would write outside the intended destination. Harden with an `fs.lstat(dest).isSymbolicLink()` check before writing. Flagged by the PR #51 automated review as a low-severity security nit; deferred, not silently fixed.
