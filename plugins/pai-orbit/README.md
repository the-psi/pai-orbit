...
`⚠️` means either "carried over as reference text only" (cursor legacy) or "partial — see cell text" (copilot agents/hooks; cursor-plugin hooks). The receiving tool's runtime capabilities determine fidelity. See each `dist/<adapter>/README.md` for specifics.
