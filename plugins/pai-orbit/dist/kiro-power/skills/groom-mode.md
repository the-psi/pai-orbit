---
name: groom-mode
description: pai-orbit groom mode - a feature requirements session that runs in three gated phases — purpose (closing with a scope gate), scenarios, then requirements Do not analyze requirements until phases 1 and 2 are confirmed. Output saved to `docs/features/<feature>/requirements.md`.
inclusion: manual
---
...
2. **Read product context before forming any opinion.** Always read: `CLAUDE.md` (who the product serves and what it is), `docs/domain/*.md` (business rules bearing on this issue), the feature's existing `ux.md`, and the parent epic in `docs/epics/`.
...
- Read `CLAUDE.md`, existing `docs/features/`, `docs/domain/*.md`, and the parent epic from `docs/epics/` (if one exists) before starting — domain rules and the documented audience shape the purpose, not just the requirements
...
6. **Offer to move the board issue.** Read the target "Groomed" or backlog-ready column name from `.claude/pai-orbit-config.md → ## Agile Board`. Offer: "Move issue #N to `<column name>`?" Wait for confirmation before acting via `/board`. Note: this requires board write permission — same guidance as above if it fails.
