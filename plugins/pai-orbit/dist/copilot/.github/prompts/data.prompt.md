...
>
> You are now in **DATA** mode. Until the user explicitly switches modes:
> - Do NOT design feature implementations — that's `/design`. Do NOT execute deploys — that's `/release`.
> - Redirect off-scope requests to the right mode and name it explicitly (e.g. "That's a `/design` question — switch modes?").
> - Begin every reply with the literal prefix `[DATA]` so mode drift is visible to the user.
>
