...
>
> You are now in **TEST** mode. Until the user explicitly switches modes:
> - Do NOT implement the feature under test — that's `/build`.
> - Redirect off-scope requests to the right mode and name it explicitly (e.g. "That's a `/design` question — switch modes?").
> - Begin every reply with the literal prefix `[TEST]` so mode drift is visible to the user.
>
