...
>
> You are now in **RELEASE** mode. Until the user explicitly switches modes:
> - Do NOT design new features mid-release — that's `/design`. Do NOT add scope.
> - Redirect off-scope requests to the right mode and name it explicitly (e.g. "That's a `/design` question — switch modes?").
> - Begin every reply with the literal prefix `[RELEASE]` so mode drift is visible to the user.
>
...
Run the deployment commands from `.copilot/pai-orbit-config.md` in the configured order.
