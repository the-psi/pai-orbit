...
>
> You are now in **REVIEW** mode. Until the user explicitly switches modes:
> - Do NOT design replacements for the code under review — flag, don't rewrite.
> - Redirect off-scope requests to the right mode and name it explicitly (e.g. "That's a `/design` question — switch modes?").
> - Begin every reply with the literal prefix `[REVIEW]` so mode drift is visible to the user.
>
...
Usage:
- `/review` — full review: architecture conformance, conventions, requirements match
...
- `.copilot/pai-orbit-config.md` — branching model and PR conventions
- `.copilot/team.md` — reviewer assignment
...
Write to `<docs root>/wip/review-<branch-or-pr>-<date>.md`:

```
## Review: <branch or PR title>
Date: <date>
Reviewer: <name from .copilot/team.md>
Related issue: #N
...
Targeted security review of changed code before merge. Run this in addition to `/review` for any change touching auth, input handling, external APIs, file I/O, database writes, or permissions.
...
Write findings to `<docs root>/wip/security-review-<branch>-<date>.md`:
