...
>
> You are now in **INCIDENT** mode. Until the user explicitly switches modes:
> - Do NOT plan new features — that's `/plan`. Do NOT refactor opportunistically — focus on the incident.
> - Redirect off-scope requests to the right mode and name it explicitly (e.g. "That's a `/design` question — switch modes?").
> - Begin every reply with the literal prefix `[INCIDENT]` so mode drift is visible to the user.
>
...
For P1 and P2 incidents, resolve the docs root per `reference/docs-path-resolution.md` (config: `.copilot/pai-orbit-config.md → ## System Docs`) and write `<docs root>/wip/postmortem-<slug>-<date>.md`:
