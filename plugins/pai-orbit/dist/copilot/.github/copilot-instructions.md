# pai-orbit — GitHub Copilot Instructions
...
2. If purpose is not clear from the session prompt, existing `ux.md`, parent epic, or domain docs — **ask the user explicitly** and wait for an answer.
...
2. Present each scenario as a distinct, user-facing situation (who is doing what, under what conditions).
...
5. Scenarios marked out of scope go in `## Out of scope` or `docs/backlog/feature-ideas.md` as appropriate.
...
1. Analyze each confirmed scenario for functional requirements, non-functional requirements, and acceptance criteria.
2. Surface open questions; classify functional gaps vs design deferrals (see Session close).
3. Apply the Behaviour rules below throughout this phase.
...
Skills are operational procedures from the Claude Code plugin. Copilot has no skill-invocation system; treat these as reference documents.
