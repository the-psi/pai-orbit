...
5. If the work is tracked in an issue, post the ADR link as a comment on the issue — use the comment-only Board Sync Checkpoint via `/board` so it works on whichever board the project uses
