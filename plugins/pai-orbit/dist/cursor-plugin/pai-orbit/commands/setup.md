...
##### `AGENTS.md` (tool-agnostic; kept under that name per D26)
...
`AGENTS.md` is tool-agnostic project documentation. The Copilot adapter's `.github/copilot-instructions.md` references it explicitly under `## Context discovery`. Do not rename it per-tool; Claude / Cursor / Copilot all read the same file.
