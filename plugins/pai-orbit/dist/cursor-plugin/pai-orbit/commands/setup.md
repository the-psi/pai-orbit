...
Resolve `{{DOCS_ROOT}}` from the Step 2 docs-home answer (item 6): the in-repo case resolves to
`docs`; a dedicated docs repo resolves to the given path; Confluence or Notion also resolves to
`docs` — those are publishing surfaces, and the local markdown mirror still lives in-repo.

Copy `templates/rules/decisions.md` to `.cursor/rules/decisions.md`, replacing `{{DOCS_ROOT}}`.
...
Copy `templates/rules/docs-taxonomy.md` to `.cursor/rules/docs-taxonomy.md`, replacing `{{DOCS_ROOT}}` throughout the routing table.
...
Rules:
- ✅ Generated — `.cursor/rules/decisions.md` — ADR obligation rules (when to write one, how)
- ✅ Generated — `.cursor/rules/docs-taxonomy.md` — which directory each artifact goes in, what `wip/` is for, when it is swept (adapt the table to this project)
