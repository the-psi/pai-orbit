...
Analyse this project's working patterns and suggest operational skills worth adding as `pai-orbit plugin skills/`. This mode **extends** Claude Code's built-in suggest-skills capability — apply all standard Claude Code suggest-skills analysis first, then layer the project-specific analysis below on top. Do not replace or skip the built-in behaviour.
