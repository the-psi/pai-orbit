...
Before starting:
- Branch: before writing any code, invoke `/git` to create and checkout a feature branch following the branching model in `.cursor/pai-orbit-config.md` → `## Git`. Derive the branch name from the feature or epic being built (e.g. `feature/<slug>` for GitHub Flow). If the current branch is already a non-protected feature branch, confirm it is the right one and proceed. If the current branch is `main`, `master`, or `develop` — and the configured model is not trunk-based — do not proceed until a feature branch is created and checked out.
