---
name: "catchup"
description: "Read-only session-start briefing. Use at the start of a work session to learn what the team shipped, what is open, what is assigned to you, and what to pick up next. Writes nothing — the briefing is printed to the conversation; hands off to build once an item is picked. Explicit invocation only."
---
...
- The briefing surfaces a prioritisation problem (stale items, conflicting sprint) → `$orbit-plan`
...
- `.codex/rules/*` — ADR rules and any other standing rules
- `.codex/pai-orbit-config.md` — board type, branching model, main/development branch, protected branches, MCP preferences, System Docs pointer
- `.codex/team.md` — roster; maps git identity to board handle
...
- Read `.codex/pai-orbit-config.md` first. If a `## System Docs` section is present, resolve `system_docs_repo` the same way `/build` and `$orbit-plan` do (relative path → check the directory exists; git URL → check for a local clone; warn once and continue if unreachable) and add `<system_docs_repo>/<system_docs_path>` to the read set.
...
Resolve `<main>` from `.codex/pai-orbit-config.md → ## Git`: the `Main branch` for GitHub Flow / trunk-based, or the `Development branch` for GitFlow. Fetch first (`git fetch --prune`) so remote refs are current.
...
- This mode contains no host- or board-specific commands. Git host and board access go through `/git` and `/board`; every project specific (handles, project keys, org URLs) is resolved at runtime from `.codex/pai-orbit-config.md` and `.codex/team.md`.
