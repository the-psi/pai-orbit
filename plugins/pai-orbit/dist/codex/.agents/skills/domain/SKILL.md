---
name: "domain"
description: "Capture domain and expert knowledge. Use when eliciting business rules, invariants, or specialist knowledge from a subject matter expert. Writes docs/domain/. Explicit invocation only."
---
...
2. **Offer to move the board issue.** If a board issue is associated with this domain work, read the next column name from `.codex/pai-orbit-config.md → ## Agile Board`. Offer: "Move issue #N to `<column name>`?" Wait for confirmation before acting via `/board`. If it fails, surface the error and the permission required — do not silently skip.
