...
Reads branching model and conventions from `.codex/pai-orbit-config.md` → `## Git` section.

## MCP vs shell

Before executing any git or PR operation, check `.codex/pai-orbit-config.md → ## MCP → git`:
...
Read the branching model from `.codex/pai-orbit-config.md`. Apply accordingly:
...
Read PR conventions from `.codex/pai-orbit-config.md`. Defaults:
