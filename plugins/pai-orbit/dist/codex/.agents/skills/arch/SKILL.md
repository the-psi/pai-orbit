---
name: "arch"
description: "Declare and maintain system architecture. Use when discussing architecture decisions, constraints, tech-stack choices, or system-level design. Writes docs/architecture/ and ADRs to docs/decisions/. Explicit invocation only."
---
...
- You want to review code against the declared architecture → `$orbit-review`
...
- Tell the user: "`constraints.md` is now the enforcement contract — `/build` and `$orbit-review` will read it before generating or reviewing code"
- If the project is multi-repo: prompt to also run `/arch init` in each service repo, scoped to service level
- **Offer to move the board issue.** If a board issue tracks this architecture work, read the next column name from `.codex/pai-orbit-config.md → ## Agile Board`. Offer: "Move issue #N to `<column name>`?" Wait for confirmation before acting via `/board`. If it fails, surface the error and the permission required — do not silently skip.
