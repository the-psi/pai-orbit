---
name: "setup"
description: "Interactive first-run scaffolding. Generates .codex/pai-orbit-config.md, .codex/team.md, .codex/config.toml, docs/ scaffold; fills lint hook repo paths and stack-builder agents. Explicit invocation only."
---
...
- Look for existing `AGENTS.md`, `.codex/pai-orbit-config.md`, `.codex/team.md` — note what is already configured
...
Write the generated agent to `.codex/agents/<service>-builder.md`. Replace all `{{PLACEHOLDER}}` markers with actual values.

### Project rules

Copy `templates/rules/decisions.md` to `.codex/rules/decisions.md`.
...
- Write `.codex/hooks/bash-guard.sh` — content from `hooks/bash-guard.sh`
- Write `.codex/hooks/arch-drift-guard.sh` — content from `hooks/arch-drift-guard.sh`

For each language detected, write the linting hooks:
- Python detected → write `.codex/hooks/lint-python.sh` from `hooks/lint-python.sh`; update any repo-specific paths
- TypeScript/JavaScript detected → write `.codex/hooks/lint-ts.sh` from `hooks/lint-ts.sh`; update any repo-specific paths
...
**Step D — Generate `.codex/config.toml`.**
...
If `.codex/config.toml` already exists, read it first and merge the `hooks` key in — do not overwrite unrelated keys. If it does not exist, create it. The hooks block to write (replace `<PROJECT_ROOT>` with the resolved absolute path):

```json
{
  "hooks": {
    "PreToolUse": [
      { "matcher": "Bash", "hooks": [{ "type": "command", "command": "<PROJECT_ROOT>/.codex/hooks/bash-guard.sh", "timeout": 10 }] }
    ],
    "PostToolUse": [
      { "matcher": "Edit|Write", "hooks": [
        { "type": "command", "command": "<PROJECT_ROOT>/.codex/hooks/lint-python.sh", "timeout": 30, "async": true },
        { "type": "command", "command": "<PROJECT_ROOT>/.codex/hooks/lint-ts.sh", "timeout": 60, "async": true },
        { "type": "command", "command": "<PROJECT_ROOT>/.codex/hooks/arch-drift-guard.sh", "timeout": 5, "async": true }
      ]}
    ]
  }
}
...
for hook in bash-guard.sh arch-drift-guard.sh; do
  [ -x "$PROJECT_ROOT/.codex/hooks/$hook" ] \
    && echo "✅ $PROJECT_ROOT/.codex/hooks/$hook" \
    || echo "⚠️  $PROJECT_ROOT/.codex/hooks/$hook — not found or not executable"
done
```
...
If the user provided MCP server answers in Step 2, write an `## MCP` section to `.codex/pai-orbit-config.md`:
...
Tell the user: "Run `/arch init` to complete your architecture declaration. Once declared, `/build` and `$orbit-review` will read `constraints.md` to enforce architectural rules automatically."
...
Rules:
- ✅ Generated — `.codex/rules/decisions.md` — ADR obligation rules (when to write one, how)

Hooks:
- ✅ Generated — `.codex/hooks/bash-guard.sh` — blocks force-push, bulk staging, hook bypass, destructive rm
- ✅ Generated — `.codex/hooks/arch-drift-guard.sh` — advisory nudge on structural file edits
- ✅ Generated (if Python) — `.codex/hooks/lint-python.sh` — runs ruff after Python edits
- ✅ Generated (if TS/JS) — `.codex/hooks/lint-ts.sh` — runs eslint after TypeScript/JavaScript edits
- ✅ Generated — `.codex/config.toml` — wires all hooks to Claude Code tool-use events

If any hook shows ⚠️ in the Step 3 validation output, surface it here with instructions: "`.codex/hooks/<name>.sh` is missing. Re-run `/setup` or manually copy the file from the pai-orbit plugin's `hooks/` directory and run `chmod +x` on it."
