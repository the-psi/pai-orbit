---
name: "incident"
description: "Investigate and record a production incident. Use for post-mortems, root-cause analysis, and remediation planning. Writes docs/reports/incident-<date>.md. Explicit invocation only."
---
...
- Fix needs review → `$orbit-review` (narrow focus: symptom, risk, rollback — not full conventions review)
...
Switch to `$orbit-review` with `focus: incident` — this narrows the review to:
...
For P1 and P2 incidents, resolve the docs root per `reference/docs-path-resolution.md` (config: `.codex/pai-orbit-config.md → ## System Docs`) and write `<docs root>/wip/postmortem-<slug>-<date>.md`:
