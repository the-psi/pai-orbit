# lint-python-wrapper.ps1 — Windows entry for lint-python.sh.
# Extracts touched paths via _extract-touched-paths.py, filters to *.py, and
# forwards each as Claude-shape JSON to the core lint-python.sh via bash.
# Advisory only. Requires python and bash on PATH.

$ErrorActionPreference = 'SilentlyContinue'

$hookDir = Split-Path -Parent $MyInvocation.MyCommand.Definition
$coreScript = Join-Path $hookDir 'lint-python.sh'
