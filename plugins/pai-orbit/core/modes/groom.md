...
2. **Read product context before forming any opinion.** Always read: `CLAUDE.md` (who the product serves and what it is), `<docs root>/domain/*.md` (business rules bearing on this issue), the feature's existing `ux.md`, and the parent epic in `<docs root>/epics/`.
...
- Read `CLAUDE.md`, existing `<docs root>/features/`, `<docs root>/domain/*.md`, and the parent epic from `<docs root>/epics/` (if one exists) before starting — domain rules and the documented audience shape the purpose, not just the requirements
