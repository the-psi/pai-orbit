...
This project's docs home is in-repo `docs/`.
...
| Impact analysis (`/analysis`) | `docs/features/<feature>/` | `analysis-<issue>-<date>.md`, or `analysis-<slug>-<date>.md` pre-ticket |
...
| Feature idea, parking lot (`/groom`) | `docs/backlog/` | `feature-ideas.md` — appended, promoted to the board by a human only |
| Domain knowledge (`/domain`) | `docs/domain/` | `domain-knowledge.md` — primary, appended with dated sections; a dedicated `<topic>.md` (e.g. `rule-engine.md`) for a large enough sub-topic, edited in place |
